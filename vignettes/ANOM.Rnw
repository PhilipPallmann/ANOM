...
%\VignetteIndexEntry{Examples using ANOM}
...
<<INSECTS>>=
insmodel1 <- glm(count ~ spray + block, data=InsectSprays, family=quasipoisson(link="log"))
...
<<INSECTS2>>=
insmodel2 <- glm(count ~ spray + block, data=InsectSprays, family=poisson(link="log"))
